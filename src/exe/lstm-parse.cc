...
//       "transition_system,s", po::value<string>()->default_value("list"),
//       "Transition system(list - listbased, spl - simplified)")(
//       "data_type,k", po::value<string>()->default_value("sdpv2"),
//       "Data type(sdpv2 - news, text - textbook)")(
...
//       "input embedding size")("hidden_dim",
//                               po::value<unsigned>()->default_value(200),
//                               "hidden dimension")(
//       "bilstm_hidden_dim", po::value<unsigned>()->default_value(100),
//       "bilstm hidden dimension")("pretrained_dim",
...
//       "LSTM input dimension")("train,t", "Should training be run?")(
//       "words,w", po::value<string>(), "Pretrained word embeddings")("help,h",
//                                                                     "Help");
...
//   explicit ParserBuilder(
//       Model &model, const unordered_map<unsigned, vector<float>> &pretrained)
...
//     if (use_bilstm) {
//       buffer_bilstm = BidirectionalLSTMLayer(model, LAYERS, LSTM_INPUT_DIM,
//                                              BILSTM_HIDDEN_DIM);
...
//     if (use_treelstm) {
//       tree_lstm =
//           TheirTreeLSTMBuilder(1, LSTM_INPUT_DIM, LSTM_INPUT_DIM, model);
...
//   const vector<int>& stacki) { if (a[1]=='W' && ssize<3) return true; if
//   (a[1]=='W') { int top=stacki[stacki.size()-1]; int
//   sec=stacki[stacki.size()-2]; if (sec>top) return true;
//     }
...
//   static bool IsActionForbidden(const string &a, unsigned bsize, unsigned ssize,
//                                 unsigned root,
//                                 const vector<vector<bool>> dir_graph,
...
//         if (b0 == (int)root &&
//             !(rel == "Root" && root_num == 0 && s0_head_num == 0))
//           return true;
//         if (b0 != (int)root && rel == "Root")
//           return true;
//       }
//       if (a[0] == 'R') {
//         if (bsize < 2 || ssize < 2)
...
//   compute_heads(const vector<unsigned> &sent, const vector<unsigned> &actions,
...
//   vector<unsigned> log_prob_parser(
//       ComputationGraph *hg,
...
//       vector<vector<string>> &cand, vector<Expression> *word_rep = NULL,
...
//     // cerr << "w2l: " << w2l.dim().ndims() << w2l.dim().d[0] << w2l.dim().d[1]
//     // << endl;
...
//     vector<int> bufferi(sent.size() +
//                         1); // position of the words in the sentence
//     // precompute buffer representation from left to right
//     vector<Expression> word_emb(
//         sent.size()); // [treelstm] store original word representation emb[i]
//                       // for sent[i]
...
//       }
//       if (use_pretrained &&
//           pretrained.count(raw_sent[i])) { // include fixed pretrained vectors?
...
//     // vector<bool> dir_graph[sent.size()]; // store the connection between
//     // words in sent
...
//         fwbuf = bilstm_outputs[sent.size() - bufferi.back()].first -
//                 bilstm_outputs[1].first;
//         bwbuf = bilstm_outputs[1].second -
//                 bilstm_outputs[sent.size() - bufferi.back()].second;
//         // [bilstm] p_t = pbias + S * slstm + P * plstm + fwB * blstm_fw + bwB *
//         // blstm_bw + A * almst
//         /*p_t = affine_transform({pbias, S, stack_lstm.back(), P,
//           pass_lstm.back(), fwB, bilstm_outputs[sent.size() -
//           bufferi.back()].first, bwB, bilstm_outputs[sent.size() -
//           bufferi.back()].second, A, action_lstm.back()});*/
//         p_t =
//             affine_transform({pbias, S, stack_lstm.back(), P, pass_lstm.back(),
...
//         p_t =
//             affine_transform({pbias, S, stack_lstm.back(), P, pass_lstm.back(),
//                               B, buffer_lstm.back(), A, action_lstm.back()});
//       }
...
//       // get relation embedding from action (TODO: convert to relation from
//       // action?)
...
//               cerr << c[i] << " ";
//             cerr << endl;*/
//             nlcomposed = tree_lstm.add_input(
//                 headi, get_children(headi, dir_graph), word_emb[headi]);
//           } else {
//             Expression composed =
...
//   void process_headless_search_all(const vector<unsigned> &sent,
//                                    const vector<unsigned> &sentPos,
//                                    const vector<string> &setOfActions,
//                                    vector<Expression> &word_rep,
//                                    Expression &act_rep, int n, int sent_len,
...
//       string rel;
//       ComputationGraph cg;
//       get_best_label(sent, sentPos, &cg, setOfActions, s0, b0, word_rep,
//                      act_rep, sent_len, dir, &score, &rel);
//       (*scores)[i] = score;
//       (*rels)[i] = rel;
...
//   void get_best_label(const vector<unsigned> &sent,
//                       const vector<unsigned> &sentPos, ComputationGraph *hg,
//                       const vector<string> &setOfActions, int s0, int b0,
//                       vector<Expression> &word_rep, Expression &act_rep,
...
//     vector<Expression> buffer; // variables representing word embeddings
//                                // (possibly including POS info)
//     vector<int> bufferi;
//     // cerr  << "ppppps 5" << endl;
...
//     // p_t = pbias + S * slstm + P * plstm + B * blstm + A * almst
//     Expression p_t =
//         affine_transform({pbias, S, stack_lstm.back(), P, pass_lstm.back(), B,
//                           buffer_lstm.back(), A, action_lstm.back()});
//     Expression nlp_t = rectify(p_t);
//     // cerr  << "ppppps 1" << endl;
//     // r_t = abias + p2a * nlp
//     Expression r_t = affine_transform({abias, p2a, nlp_t});
...
// map<string, double> evaluate(const vector<vector<vector<string>>> &refs,
...
// int process_headless(vector<vector<string>> &hyp, vector<vector<string>> &cand,
//                      vector<Expression> &word_rep, Expression &act_rep,
//                      ParserBuilder &parser, const vector<string> &setOfActions,
...
//         map<int, string> rels;
//         parser.process_headless_search_all(
//             sent, sentPos, setOfActions, word_rep, act_rep, i,
...
//   assert(unk_prob <= 1.);
//   ostringstream os;
//   os << "parser_" << (USE_POS ? "pos" : "nopos") << '_'
//      << (use_bilstm ? "bilstm" : "nobilstm") << '_'
//      << (use_treelstm ? "treelstm" : "notreelstm") << '_'
//      << conf["data_type"].as<string>() << '_' << LAYERS << '_' << INPUT_DIM
//      << '_' << HIDDEN_DIM << '_' << ACTION_DIM << '_' << LSTM_INPUT_DIM << '_'
//      << POS_DIM << '_' << REL_DIM << "-pid" << getpid() << ".params";
//   double best_LF = 0;
//   double best_NLF = 0;
...
//   POS_SIZE = corpus.npos + 10; // bad way of dealing with the fact that we may
//                                // see new POS tags in the test set
...
//         min(status_every_i_iterations, corpus.nsentences);
...
//     // cerr << "TRAINING STARTED AT: " << asctime(localtime(&time_start)) <<
//     // endl;
...
//         ComputationGraph hg;
//         // cerr << "Start word:" <<
//         // corpus.intToWords[sentence[0]]<<corpus.intToWords[sentence[1]] <<
//         // endl;
...
//           ComputationGraph hg;
//           vector<vector<string>> cand;
//           vector<unsigned> pred = parser.log_prob_parser(
...
//               parser.compute_heads(sentence, actions, corpus.actions);
//           vector<vector<string>> hyp =
//               parser.compute_heads(sentence, pred, corpus.actions);
...
//             evaluate(refs, hyps, corpus.sentencesPosDev, corpus.posToInt["PU"]);
...
//       vector<vector<string>> hyp =
//           parser.compute_heads(sentence, pred, corpus.actions);
//       if (process_headless(hyp, cand, word_rep, act_rep, parser, corpus.actions,
...
//          << " UF:" << results["UF"] << " NLF: " << results["NLF"]
//          << " NUF:" << results["NUF"] << " LP:" << results["LP"]
