...
TheirTreeLSTMBuilder::TheirTreeLSTMBuilder(unsigned layers, unsigned input_dim,
  ...
  for (unsigned i = 0; i < layers; ++i) {
    ...
    layer_input_dim =
        hidden_dim; // output (hidden) from 1st layer is input to next
...
void TheirTreeLSTMBuilder::new_graph_impl(ComputationGraph &cg, bool update) {
  param_vars.clear();
  ...
  for (unsigned i = 0; i < layers; ++i) {
    ...
    vector<Expression> vars = {i_x2i, i_h2i, i_bi, i_x2f, i_h2f, i_bf,
                               i_x2o, i_h2o, i_bo, i_x2c, i_h2c, i_bc};
...
Expression TheirTreeLSTMBuilder::add_input(int idx, vector<unsigned> children,
  ...
  for (unsigned layer = 0; layer < layers; ++layer) {
    ...
    Expression i_at;
    if (has_children) {
      i_at = affine_transform({vars[BI], vars[X2I], in, vars[H2I], i_h_k_sum});
    } else {
    ...
    if (has_children) {
      for (unsigned k = 0; k < children.size(); k++) {
        Expression i_aftk =
            affine_transform({vars[BF], vars[X2F], in, vars[H2F], i_h_k[k]});
    ...
    Expression i_act;
    if (has_children) {
      i_act = affine_transform({vars[BC], vars[X2C], in, vars[H2C], i_h_k_sum});
    } else {
    ...
    Expression i_aot;
    if (has_children) {
      i_aot = affine_transform({vars[BO], vars[X2O], in, vars[H2O], i_h_k_sum});
    } else {
...
void TheirTreeLSTMBuilder::copy(const RNNBuilder &rnn) {
  const TheirTreeLSTMBuilder &rnn_treelstm = (const TheirTreeLSTMBuilder &)rnn;
