...
struct TheirTreeLSTMBuilder : public RNNBuilder {
  ...
  Expression add_input(int idx, vector<unsigned> children, const Expression &x);
...
protected:
  ...
  Expression inline set_h_impl(int prev,
                               const std::vector<Expression> &h_new) override {
    cerr << "set_h_impl is not supported." << endl;
    throw new std::runtime_error("set_h_impl is not supported.");
  }
  Expression inline set_s_impl(int prev,
                               const std::vector<Expression> &s_new) override {
    cerr << "set_s_impl is not supported." << endl;
    throw new std::runtime_error("set_s_impl is not supported.");
  }
